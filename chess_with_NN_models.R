...
preds = predict(mod.glm, newdata = valid.na, type = "response" )
print(discrepancy(as.numeric(valid$lapsed)-1,preds))
...
model <- train(lapsed~., data=train.na, method='nnet', 
               trControl=ctrl ,
               number = 1,
               tuneLength = 1,
...
model
probs <- predict(model, valid.na,type='prob')
probs
...
nn <- train(train.predictors.standardized[1:500,], train.na[1:500,1], method='nnet', 
            trControl=trainControl(method='cv'))
nn
valid.preds=predict.train(nn,newdata=valid.predictors.standardized,type='prob')
print(discrepancy(as.numeric(valid$lapsed)-1,valid.preds[,2]))

# Now train on full train set
nn.full <- train(train.predictors.standardized, train.na[,1], method='nnet', 
                 trControl=trainControl(method='cv'))
nn.full
valid.preds2=predict.train(nn.full,newdata=valid.predictors.standardized,type='prob')
...
# Train NN Regressor on all data maximizing Log Loss
trainctrl = trainControl(method="cv", number=4,repeats=1,classProbs = TRUE,
                         summaryFunction = twoClassSummary)
nn.grid <- expand.grid(decay = c(.1,.01), size=c(3,5))
nn.full2 <- train(train.predictors.standardized, 
                train.na[,1], method='nnet',
                 metric='logLoss',
                 tuneGrid=nn.grid,
                 trControl=trainctrl,
                 maxit = 200)
...
#Predict on test set
preds = predict.train(nn.full2, newdata = test.predictors.standardized, type = "prob" )
 
#Save predictions
prediction = write.csv(x = preds[,2],file = "predictions_neural_network_classifier2.csv")
